...
#print('Numero de pontos: ', len(qtree))
...
center_x = int ( input("ponto inicial horizontal: "))
center_y = int (input("ponto inicial vertical: "))

range_width = int (input("distancia horizontal: "))
#np.random.rand() * min(center_x, width - center_x)
range_height = int (input("distancia vertical: "))
#np.random.rand() * min(center_y, height - center_y)

found_points = []
range = Rectangle(points[5], range_width, range_height)
#range = Rectangle(Point(center_x, center_y), range_width, range_height)
found_points = qtree.queryrange(range)
radius = 150

print('pontos no alcance:', len(found_points))
